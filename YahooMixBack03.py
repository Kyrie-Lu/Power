...
def backtest(signals, cash, port_value =.5, batch =100):
    ...
    results =  pd.DataFrame({"Start Cash": [],
                            ...
                            "Profit": [],
                            "ValueAll":[]
                            })  #Share Value 每次买卖的价值，Profit该次交易的利润
    #stockValue = 0
 
    for index, row in signals.iterrows():
        # These first few lines are done for any trade
        #shares = portfolio.setdefault(index[SYMBOL], 0)  #portfolis已经改变   #需要处理        
        #print("aa+",portfolio,"\n")
        #print("bb+",type(shares),"\n")
        trade_val =0
        batches =0
        cash_change = 0    #并不是一直为0
        ...
        if row["Signal"] =="Buy" and row["Regime"] ==1 :  # Entering a long position
            ...
            #old_price = row['Price']
            print("buy",portfolio[index[SYMBOL]],"   ",port_prices[index[SYMBOL]],"\n")
        elif row["Regime"] == -1 and portfolio[index[SYMBOL]] != 0: # Entering a short
            #print(portfolio[index[SYMBOL]],index[SYMBOL0],'\n')
            ...
            print("sell",portfolio[index[SYMBOL]],"\n")
            #stockValue -= trade_val
        ...
        pprofit = row["Price"] - port_prices[index[SYMBOL]]   # Compute profit per share; old_price is set in such a way that entering a position results in a profit of zero
        Profit = (trade_val / row["Price"]) * pprofit
        cashall += Profit
        # Update report
...
bb = backtest(signals, cash=1000000, port_value =.8, batch =100)
pd.set_option('display.max_columns', None) 
print(bb)
